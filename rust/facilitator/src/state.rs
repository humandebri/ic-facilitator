...
impl SettlementRecord {
    pub fn checking(payer: String, amount: String, now: u64, ttl: u64) -> Self {
        Self::new(
            "checking",
            pending_response(
                "",
    ...
    pub fn broadcast(tx: String, payer: String, amount: String, now: u64, ttl: u64) -> Self {
        Self::new(
            "broadcast",
            pending_response(
                &tx,
    ...
    pub fn settled(tx: String, payer: String, amount: String, now: u64, ttl: u64) -> Self {
        Self::new(
            "settled",
    ...
    pub fn failed(tx: String, message: String, payer: String, now: u64, ttl: u64) -> Self {
        let mut response = failed_settlement(NETWORK, "settlement_failed", &message, Some(payer));
        response.transaction = tx;
        Self::new("failed", response, now, ttl)
    }
    ...
    fn new(status: &str, response: SettleResponse, now: u64, ttl: u64) -> Self {
        Self {
...
mod tests {
    ...
    fn maps_record_status_to_http_status() {
        let checking = SettlementRecord::checking("0xabc".to_string(), "100".to_string(), 10, 60);
    ...
    fn detects_expired_records() {
        let record = SettlementRecord::checking("0xabc".to_string(), "100".to_string(), 10, 60);
