...
import { healthMatchesWebConfig } from "../runtime_config";
...
export function Onboarding() {
  ...
  return <section className="page narrow"><PageHeader eyebrow="SELLER ONBOARDING" title="署名して利用を開始" lead="3つの文書と対象networkを確認し、seller walletで同意を記録します。"/>{error&&<Notice tone="warning">{error}</Notice>}{health&&!versionsMatch&&<Notice tone="warning">法務文書versionがfacilitator設定と一致しないため、署名を停止しています。</Notice>}{health&&!runtimeMatches&&<Notice tone="warning">Webとfacilitatorのnetwork・contract設定が一致しないため、署名を停止しています。</Notice>}{result?<Notice tone="success">登録が完了しました。受付時刻: {new Date(result.acceptedAt*1000).toLocaleString("ja-JP")}</Notice>:<div className="step-card"><div className="step-row"><span>1</span><div><h2>Wallet</h2>{wallet?<code>{wallet.address}</code>:<button className="button" onClick={() => void connect()}>Rabby / MetaMaskを接続</button>}</div></div><div className="step-row"><span>2</span><div><h2>Network</h2><p>{health?.networkProfile ?? config.environment} · Chain {health?.chainId ?? config.chainId}</p></div></div><div className="step-row"><span>3</span><div><h2>文書を確認</h2>{docs.map(([path,label,version],i)=><label className="check" key={label}><input type="checkbox" checked={checks[i]} disabled={!versionsMatch||!runtimeMatches} onChange={e=>setChecks(v=>v.map((x,j)=>j===i?e.target.checked:x))}/><span><a href={String(path)} target="_blank">{label}</a> <small>version {version??"draft"}</small></span></label>)}</div></div><button className="button full" disabled={!wallet||!checks.every(Boolean)||busy||!versionsMatch||!runtimeMatches} onClick={() => void accept()}>{busy?"署名を確認中…":"同意に署名して登録"}</button></div>}</section>;
}
