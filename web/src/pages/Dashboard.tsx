...
export function Dashboard() {
  ...
  return <section className="page"><SellerNav/><PageHeader eyebrow="SELLER DASHBOARD" title="決済の準備状況" lead="同意、Credit、Facilitatorの基本設定を一か所で確認できます。"/>
    {!address&&<button className="button" onClick={()=>void load()}>Seller walletを接続</button>}{error&&<Notice tone="warning">{error}</Notice>}
    {address&&<><div className="identity"><div><span>Seller</span><Address value={address}/></div><span className={`status ${health?.readiness?"ready":"waiting"}`}>{health?.readiness?"基本設定済み":"設定確認が必要"}</span></div>
      <div className="stats"><Stat label="Seller Credit" value={formatJpyc(credit)} note="Facilitator手数料専用"/><Stat label="Exact料金" value={formatJpyc(health?.sellerSettlementFeeAmount)}/><Stat label="Batch料金" value={formatJpyc(health?.batchSettlementFeeAmount)}/></div>
