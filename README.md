...
- update `batch_set_seller(receiver, status)`, `batch_set_writer_receiver_scope(writer, receiver, enabled)`: controller管理API。network profile、token、Batch contract、Exact/Batch action料金、任意の8段階schedule、3文書versionは`set_runtime_configuration(record)`で検証後に一括更新する。旧`set_runtime_profile`とfee setterは互換用に残す非推奨APIであり、新規運用では使用しない。

## 利用者向け

このfacilitatorは、JPYCのExact決済とBatch決済を処理します。Batchでは複数のclaimをまとめて処理でき、1回のリクエストで最大100件まで利用できます。

### 料金

以下は利用者向けの正規料金です。料金はseller creditからfacilitator利用料として支払われます。

| Action | 料金 |
| --- | ---: |
| Exact | 0.5 JPYC |
| Batch deposit | 0.5 JPYC |
| Batch claim（1件） | 件数別tier料金 |
| Batch claim（2〜10件） | 件数別tier料金 |
| Batch claim（11〜50件） | 件数別tier料金 |
| Batch claim（51〜100件） | 件数別tier料金 |
| Batch settle | 0.5 JPYC |
| Batch refund（claimsなし） | 0.5 JPYC |
| Batch refund（claimsあり・1件） | 件数別tier料金 |
| Batch refund（claimsあり・2〜10件） | 件数別tier料金 |
| Batch refund（claimsあり・11〜50件） | 件数別tier料金 |
| Batch refund（claimsあり・51〜100件） | 件数別tier料金 |
| すでにsettle済みのno-op settle | 無料 |

seller creditは1 JPYC以上をtop-upできます。claimとclaims付きrefundは件数に応じた料金tierを適用し、1リクエストの上限は100件です。101件以上は複数のリクエストに分けてください。
...
以下はfacilitatorを運用・検証する人向けの設定です。利用者は上の「利用者向け」だけを確認してください。
...
SELLER_SETTLEMENT_FEE_AMOUNT=500000000000000000
...
# Legacy fallback only. Action-specific fees below are canonical.
BATCH_SETTLEMENT_FEE_AMOUNT=5000000000000000000
BATCH_DEPOSIT_FEE_AMOUNT=500000000000000000
BATCH_CLAIM_FEE_AMOUNT=5000000000000000000
BATCH_SETTLE_FEE_AMOUNT=500000000000000000
BATCH_REFUND_FEE_AMOUNT=500000000000000000
# 件数別料金。8項目を全て設定した場合のみ有効（Amoy実測＋Mainnet gas条件で決定）。
...
Webのpreview・production buildでは`VITE_TOKEN_ADDRESS`と`VITE_BATCH_SETTLEMENT_CONTRACT`を必須とする。どちらも非zeroのEVM addressを設定し、Onboarding開始時にcanister healthのtoken・batch contractと一致しない場合は署名操作を無効化する。
...
mainnet公開はこのMVPの対象外であり、`npm run ic:deploy:mainnet`は設定検証後に必ず停止する。既存canisterは旧payment-intent rowを監査・解消するtransition releaseなしに直接upgradeしない。Production手順を有効化する際は、profile・token・contract・料金・承認済み3文書versionを`set_runtime_configuration`で原子的に反映し、readiness checklistをすべて満たすことを別リリースで確認する。

upgrade前はcanisterを停止し、outstanding callbackが排出された状態でinstallし、再開後に10分以上残った未送信`checking`だけをcontrollerが`recover_stale_settlement(key)`で回復する。このAPIは記録済み手数料を一度だけ返金し、active lockと履歴索引を解放する。broadcast済みrecordや稼働中canisterのcallbackをTTLで強制回収してはならない。
